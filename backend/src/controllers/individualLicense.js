...
module.exports = {
    add: async(req, res) => {
        try {
            ...
            await docDutyMan.save()
            .then(docs => {
                return doc.save()
            }).then(d => {
                res.status(201).json({message: 'Individual License Added Succesfully'});
            })
            .catch(err => {
                console.log(err);
    ...
    update: async(req, res) => {
        IndividualLicense.updateOne({ refUser: req.params.id },{ $set: req.body }, { new: true })
    ...
    delete: async(req, res) => {
        await IndividualLicense.updateOne({ _id: req.params.id }, { $set: { isActive: false } }, { new: true }).then(docs => {
            return DutyManagers.updateMany({ certId: req.params.id }, { $set: { isActive: false } }, { new: true })
        }).then(d => {
            res.status(201).json({message: 'Individual License Deleted Succesfully'});
        }).catch(err => {
            res.status(500).json({ error: "Internal server error" });
        });
    }
